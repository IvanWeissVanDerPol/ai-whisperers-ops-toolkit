...
# 2. Send Messaging notifications for due/overdue tasks (3 boards: ivan/kiki/lua)
# 3. Process incoming Messaging DONE replies (3 boards)
...
# 2. Messaging due/overdue notifications — all three people
for board in ivan-tasks kiki-tasks lua-tasks; do
    python3 /root/.hermes/scripts/kanban_messaging_notify.py \
        --board "$board" --mode overdue >> "$LOG" 2>&1
done

# 3. Process incoming Messaging DONE replies — all three boards
for board in ivan-tasks kiki-tasks lua-tasks; do
    python3 /root/.hermes/scripts/kanban_messaging_done_handler.py \
        --board "$board" >> "$LOG" 2>&1
done
