...
**Date:** 2026-07-29
**Source:** `cursor_20260628-2.zip` (1.9 MB, **620 files** — the full [Engagement A] playbook)
...
[Engagement A] engineering playbook** — the AI rules + prompts + skills + scripts +
...
|---|---|---|
| `prompt-quality-rubric` | [Engagement A] `validate-prompt.prompt.md` rubric | ✅ |
| `prompt-improvement-loop` | [Engagement A] `prompt-improvement-loop.json` engine | ✅ |
...
| [Engagement A] skill | What it does | Hermes equivalent today |
|---|---|---|
...
[Engagement A] uses a **meta-pattern** that I missed entirely in Round 3:
...
Every [Engagement A] rule has a **structured frontmatter** that defines its
contract:
...
[Engagement A] ran a self-audit in `rules/CROSS-DOMAIN-INVOCATION-VALIDATION.md`
and found:
...
**Insight:** Even [Engagement A]'s own playbook has compliance gaps. Hermes would
...
Every [Engagement A] prompt has richer frontmatter than our skills:
...
templar: .REPLACE_ME.md
exemplar: .REPLACE_ME.md
rules:
...
**Most transferable to Hermes (not [Engagement A]-specific):**
...
**[Engagement A]-specific (drop):** migration (C++→C#), azure devops, .NET diagnostics.
...
**Estimated effort:** 6 hours
**Value:** Closes the prompt-library gap. [Engagement A] has 188 prompts;
Hermes has 0 (we only have skills).

Port the 6 highest-value [Engagement A] prompts into Hermes-style skills:

1. `code-quality/review-code-quality.prompt.md` → `~/.hermes/skills/code-review/`
2. `REPLACE_ME.prompt.md` → `~/.hermes/skills/find-dead-skills/`
3. `REPLACE_ME.prompt.md` → `~/.hermes/skills/doc-architecture/`
...
1. **[Engagement A] is .NET / Azure DevOps.** Roughly 60% of the material is
   [Engagement A]-specific (CS/IDE/CA diagnostics, .csproj, Azure Pipelines,
