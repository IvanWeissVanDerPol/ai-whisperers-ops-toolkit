...
**Source:** `cursor_20260628-2.zip` (1.9 MB, **620 files** — the full [Engagement A] engineering playbook)
...
### The 6 Ported [Engagement A] Prompts (P7)

Each one is a Hermes-flavored adaptation of an [Engagement A] prompt that
matched their gold standard:

| Skill | Source ([Engagement A]) | Adapted for |
|---|---|---|
| `code-review-exemplar` | `.REPLACE_ME.md` | PR review with 6-severity cal |
| `find-dead-code` | `.REPLACE_ME.prompt.md` | dead skills, dead scripts, dangling refs |
| `changelog-releaser` | `.REPLACE_ME.prompt.md` + `release/check-release-branch.prompt.md` | version-bump + tag from git |
| `api-refactor` | `.REPLACE_ME.prompt.md` | safe public-API refactor with deprecation |
| `doc-architecture` | `.REPLACE_ME.prompt.md` | 3-folder pattern + Mermaid templates |
...
| Skill | What it does |
|---|---|
| `skill-frontmatter-schema` | Canonical frontmatter schema (REPLACE_ME) |
...
| Ported [Engagement A] prompts | 0 | 6 |
...
The router pattern [Engagement A] uses, now implemented in Hermes:
...
Adopted from [Engagement A]'s `manage-playbook` + `process-prompt-improvement-cycle`:
...
```bash
python3 ~/.REPLACE_ME.py --skill ~/.hermes/skills/<skill-name>
```
...
provenance:
  ...
  source: "cursor_20260628-2.zip — <exact [Engagement A] path>"
...
From [Engagement A]'s `rule-authoring/` framework, applied to Hermes:
...
5. **The 6 ported [Engagement A] prompts are slimmer than the originals.**
   [Engagement A]'s prompts are 200-900 lines each. Our Hermes adaptations are
   200-400 lines focused on the core pattern. Future rounds can
   expand them with the full [Engagement A] exemplar text.
