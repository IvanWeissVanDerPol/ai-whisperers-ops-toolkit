...
[Engagement A]'s `ticket-plan` was a thin router (3 operations: validate-plan, fix-plan, roadmap). Hermes's `ticket-lifecycle` already covered start→plan→execute→progress→validate→close but lacked these 3 pre-execution / multi-subticket phases.
...
**Fix:** Created `REPLACE_ME.md` (9.0KB) with:
...
| **Real-world incident modes** | 4 documented [Engagement A] incidents with recovery time (2 days, 4 hours, 6 hours, 1 day) |
...
| Aspect | Detail |
|---|---|
| **Source pattern** | [Engagement A] `.cursor/skills/jp-toolchain/SKILL.md` — router for 7 manual `/jp-*` commands |
...
| **Why skip** | All 11 dirs are EMPTY — no SKILL.md inside. They were placeholders for the "file_pattern → rule" router pattern that [Engagement A] never finished implementing. |
| **Verdict** | Creating empty stubs to match empty stubs would be cargo-culting. The pattern itself (file_pattern → rule) is captured in `ticket-lifecycle`'s "Agent-Application Pattern" section. The 11 specific stubs were [Engagement A]-domain-specific (dotnet, rule-authoring, scripts) and don't translate to a greenfield TS/Python stack. |
...
| FORBIDDEN list | `REPLACE_ME.md` | ✓ R11 |
| 123 rules | partial via existing skills | ⚠️ most are [Engagement A]-style enforcement, don't generalize |
| 104 exemplars | partial via existing skills | ⚠️ most are [Engagement A]-specific |

**Coverage: ~95% of high-value patterns now integrated.** The remaining 5% is genuinely [Engagement A]-specific (Jira tickets, C#/.NET, VB.NET migration, [Engagement A]-ticket-prefix) and doesn't generalize to a greenfield TS/Python/AI-agent stack.
...
✓ REPLACE_ME.md: NEW (7.1KB)
✓ REPLACE_ME.md: NEW (5.0KB)
✓ REPLACE_ME.md: NEW (5.1KB)
